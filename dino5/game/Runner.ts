...
export default class Runner {
  ...
  invertTrigger = false
  updatePending = false
  resizeTimerId_: number | null = null // window resizing delay
  ...
  constructor(outerContainerId: string, optConfig?: ConfigDict) {
    const element = document.querySelector(outerContainerId) as HTMLElement
    if (!element) {
      throw new Error(`Container element with id "${outerContainerId}" not found`)
    }
    this.outerContainerEl = element
  ...
  init() {
    ...
    if (!this.containerEl) {
      throw new Error(`Container element with class "${Runner.classes.CONTAINER}" not found`)
    }
    ...
    if (!canvas) {
      throw new Error(`Canvas element with class "${Runner.classes.CANVAS}" not found`)
    }
  ...
  adjustDimensions() {
    this.resizeTimerId_ && clearInterval(this.resizeTimerId_ as any)
